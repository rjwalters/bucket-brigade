%% main.tex --- Bucket Brigade workshop paper (v2 draft)
%%
...
%% The TikZ figure in figures/recalibrated_heatmap.tex requires the tikz
%% and pgfplots-compatible pattern library, which are loaded below alongside
%% the class defaults.
...
design, and is not a substitute for any of them.

We treat the analytical NE characterization as ground truth and the PPO
trainability sweep as a falsification test, in that order.
...
tractability that motivated the reduction; we report the discrepancy openly
and name the analytical extensions (ring-Markov refinement; per-agent
ownership in the survival coefficient) as the natural follow-on work in
...
no-pure-NE collapse regime at small~$\kappa$---all of which are
non-trivially observable in the 7-cell preview and none of which would be
derivable without the closed-form derivation. The quantitative
$\kappa$-thresholds are off by $3$--$10\times$. Honest disagreement at this
boundary is the paper's claim, not a weakness we hide. The qualitative
phase order furthermore transfers to the PPO trainability sweep under a
per-cell NE-anchored metric (\S\ref{sec:trainability}), bearing out the
analytical prediction at the algorithm-evaluation layer. Detailed
derivations, the $7\times 4$ predicted-vs-empirical table, and the five
sources of systematic bias appear in~\citep{bbnestructure}.
...
$0.017$, well within seed variance, confirming the
$\beta$-independence prediction of~\eqref{eq:S}--\eqref{eq:C} at the PPO
level as well as at the equilibrium-solver level. PPO recovers
...
A methodological observation worth recording: under the original
single-cell baseline of the v1 draft, the ordering inverted to
...
\begin{figure}[t]
  \centering
  \input{figures/recalibrated_heatmap.tex}
...
introducing a new benchmark despite the trade. We do not claim Bucket
Brigade scales to large populations; we do not claim it tests
``emergent cooperation'' in any general sense; we do not claim it is a
general-purpose MARL benchmark. It is none of these things. The niche it
fills is the specific question of \emph{equilibrium-convergence quality}.
...
not fully separated by the 7-cell preview; rigorous separation requires the
...
extension. The empirical contribution is also contingent: if the full phase
diagram turns out to be uninteresting (one NE structure dominates
everywhere), the workshop pitch weakens substantially. The 7-cell preview
already shows non-trivial structure, but the full-grid result is the gate.
...
column of Figure~\ref{fig:ppo} can be re-derived from the raw
\#360-sweep checkpoints.
