%% main.tex --- Bucket Brigade workshop paper (v7 draft)
%%
%% v7 integrates the July-2026 session results (repo issue #446; PRs #431,
%% #432, #433, #438, #439, #440, #441; issues #434, #436, #442, #443):
%%   - \S4: new "Per-cell trainability predictors" paragraphs reporting the
%%     retired conditional-entropy predictor (Spearman rho = 0.007, p = 0.97;
%%     experiments/nash/phase_diagram/entropy_vs_trainability.md) and the
%%     beta-invariance argument that NO statistic of the NE profile can
%%     explain within-(kappa, c)-column trainability variance. The
%%     coordination-threshold k* hypothesis is stated with explicit
%%     "pending" status (repo issue #443; rjwalters/thrust#269).
%%   - New \S5 (social-trap hardness): rest_trap-v1 four-anchor table
%%     (NE <= 248.67 < random 302.87 < best-PPO 306.26 < scripted 386.60),
%%     the scenario-aware gap_closed degenerate-reference mechanism (#434),
%%     the categorical trap-escape verdict rule (#436), and the honest
%%     het_ppo reading: `at_random`, never a gate pass.
%%   - \S3/\S4/abstract/conclusion/Appendix B: beta-independence claims
%%     corrected per issue #442 -- beta is provably inert in the bernoulli
%%     extinguish mode the sweep uses (spread never fires; zero RNG draws),
%%     so verdict-level beta-invariance is guaranteed by construction and
%%     residual cross-beta differences measure double-oracle solver noise.
%%   - \S6: Reproducibility expanded -- frozen -v1 scenario registry
%%     (PR #441), parity manifest + CLI (PR #439, docs/PARITY.md), and the
%%     verified ~7x downstream reward-scale mismatch (PR #432) as the
%%     motivating incident.
%% The stale-claim audit table for this revision is committed alongside at
%% stale_claims_audit.md.
%%
%% v6 landed the v5-reviewer/auditor punch list (categorical Figure 1 with
%% source script, Table 2 margin fix, \S4 metric-consistent separations).
%% v5 inlined the memo placeholders as Appendices A/B. v7 preserves all of
%% those changes.
%%
...
the NE profile can explain the within-column variance; the replacement
coordination-threshold ($k^{*}$) hypothesis is stated with its validation
explicitly pending. On the social-trap scenario \texttt{rest\_trap-v1},
whose frozen equilibrium is team-suboptimal by construction, an
all-scripted specialist team scores $386.6$/step where $20$-seed PPO
reaches $306.3$~--~statistically indistinguishable from uniform-random
play ($302.9$)~--~a machine-verdicted ${\approx}80$/step learnability gap
that gives the benchmark a measured, currently-uncaptured headroom
target. The quantitative
...
profile as an explanation of within-$(\kappa,c)$-column variance; the
replacement coordination-threshold ($k^{*}$) hypothesis is stated with
its validation explicitly pending.
...
$386.60$/step against a $302.87$/step uniform-random baseline, while the
best $20$-seed PPO run is statistically indistinguishable from
random~--~a machine-verdicted ${\approx}80$/step learnability gap,
grounded in a four-anchor verdict rule rather than an ad-hoc fraction
gate. (5)~An honest
...
$12/13$ rows, are more robust. The grid surfaces \emph{four}
...
deviation is required. We state this as a hypothesis, not a finding: the
per-cell $k^{*}$ artifact is produced by a downstream oracle line that
has not landed (\texttt{rjwalters/thrust\#269}), and the $20$-seed noise
buy-down on a $k^{*}$-stratified cell subset needed to lift the $n{=}4$
noise ceiling is an open issue in the source repository (repo
issue~\#443). The account must also contend with an awkward datum: a
...
predictor claim; the retirement of the entropy predictor and the
$\beta$-invariance impossibility argument are the results we stand
behind.
...
makes with committed measurements: \emph{the best trained policy is
statistically indistinguishable from random play, while a hand-coded
scripted team demonstrates ${\approx}80$/step of untouched headroom.}
...
double-oracle solver noise and a better equilibrium may exist; the trap
\emph{ordering} is nonetheless robust, because the fully scripted
\texttt{always\_rest} team also scores below random
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    Best trained (\texttt{het\_ppo}, $20$ seeds) & $306.26$ $[302.95, 309.33]$
...
\paragraph{The honest trained-policy result: \textsf{at\_random}.}
The strongest tier-1 result on \texttt{rest\_trap-v1} is heterogeneous
...
\emph{no} positive interventional evidence on this benchmark; we state
...
(${\leq}248.67 < 302.87 < 306.26 < 386.60$ per step) in which the next
rung---\textsf{escaped\_trap}---has a precise statistical meaning, and
the full ${\approx}80$/step gap to \texttt{scripted\_best} is known to
be realizable by a stationary policy profile.
...
\S\ref{sec:trainability}---with the coordination-threshold $k^{*}$
hypothesis stated and its validation explicitly pending. On the
social-trap scenario \texttt{rest\_trap-v1}, the benchmark's hardest
committed target, the best $20$-seed trained policy is statistically
indistinguishable from random play while a scripted specialist team
