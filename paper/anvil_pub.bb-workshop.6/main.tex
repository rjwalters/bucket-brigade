%% main.tex --- Bucket Brigade workshop paper (v6 draft)
%%
%% v6 lands the v5-reviewer and v5-auditor punch list:
%%   - Figure 1 regenerated as a categorical 3-panel regime heatmap with a
%%     real source script at figures/src/phase_diagram.py. Replaces the
%%     v5 carry-forward PNG that showed raw payoff values instead of
%%     regime classes.
%%   - Table 2 (benchmark comparison) tabcolsep + cell-text tightened so
%%     the "Coop/comp" column no longer overflows the right margin.
%%   - \S3 cross-reference to "per-cell predicted-vs-empirical table in
%%     Appendix B" softened to match what Appendix B actually carries.
%%   - \S4 std-range estimator named: "per-class mean within-cell std
%%     (0.21-0.44)" so the ordering-not-significance sentence is
%%     unambiguous.
%%   - \S4 class-mean separations switched to the metric-consistent
%%     homogeneous numbers (0.001, 0.017, 0.082); v5's (0.073, 0.048,
%%     0.108) crossed gap_closed_ne and gap_closed_homogeneous at the
%%     asymmetric -> collapse separation.
%%
%% v5 already inlined the bbenvspec / bbnestructure memo placeholders as
%% Appendix A (env spec) and Appendix B (NE characterisation), split
%% \S1 contributions into (3a)/(3b), added the per-kappa Table 1 in \S3,
%% merged the two Threats paragraphs in \S6, and tightened Figure 1/2
%% captions. v6 preserves all of those changes and lands only the v5
%% punch list above.
%%
...
\begin{figure}[t]
  ...
  $(\kappa{=}0.9, c{=}0.5)$), matching the prediction
  in~\eqref{eq:S}--\eqref{eq:C} but at substantially different
...
scope for the camera-ready and is the natural significance gate. The $\beta$-independence prediction holds at verdict
