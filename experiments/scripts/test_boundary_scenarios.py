...
def main():
    ...
    for i, (scenario_name, data) in enumerate(sorted_results, 1):
        ...
        print(
            f"{i:2d}. {scenario_name:20s} β={beta:.2f} c={c:.2f} → {payoff:6.2f}{tag}"
        )
    ...
    print(
        f"Worst Performance: {worst[0]} (β={worst[1]['beta']:.2f}, c={worst[1]['c']:.2f}) → {worst[1]['payoff']:.2f}"
    )
    print(
        f"Best Performance:  {best[0]} (β={best[1]['beta']:.2f}, c={best[1]['c']:.2f}) → {best[1]['payoff']:.2f}"
    )
    ...
    print(
        f"Degradation:          {baseline_mean - extreme_mean:.2f} ({(baseline_mean - extreme_mean) / baseline_mean * 100:.1f}%)"
    )
    print()
    ...
    failures = [
        (name, data) for name, data in results.items() if data["payoff"] < threshold
    ]
    ...
    if failures:
        ...
        for name, data in failures:
            print(
                f"  - {name}: {data['payoff']:.2f} (β={data['beta']:.2f}, c={data['c']:.2f})"
            )
    else:
        ...
        json.dump(
            {
                ...
                "summary": {
                    ...
                    "degradation_pct": (baseline_mean - extreme_mean)
                    / baseline_mean
                    * 100,
                    "worst": worst[0],
                    "best": best[0],
