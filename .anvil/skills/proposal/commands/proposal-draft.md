...
**Role**: drafter.
**Reads**: `<thread>/BRIEF.md` (if present), `<thread>/refs/**` (if present), any `<thread>.0.perspective/` sibling (optional load-bearing context if present — see "Optional perspective context" in the Inputs section), and the `templates/proposal.tex.j2` + `templates/anvil-proposal.cls` shipped with this skill. For revise-from-feedback path: also the latest `<thread>.{N}/` and all `<thread>.{N}.*/` critic siblings.
