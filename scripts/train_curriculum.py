...
class CurriculumTrainer:
    ...
    def train_curriculum(self, run_name: str = 'curriculum_training'):
        ...
        # Initialize policy
        dummy_env = make_vectorized_env(scenario_name='trivial_cooperation', num_opponents=2)
        ...
        for stage_idx, stage in enumerate(self.curriculum):
            ...
            env = make_vectorized_env(
                ...
                num_opponents=stage['opponents']
            )

            # Train on this stage
            stage_timesteps = self._train_stage(
                policy, env, stage, run_name, stage_idx
            )
...
def main():
    parser = argparse.ArgumentParser(description='Train Bucket Brigade agent with curriculum learning')

    # Curriculum settings
    parser.add_argument('--run-name', type=str, default='curriculum_training',
