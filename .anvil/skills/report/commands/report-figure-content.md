---
name: report-figure-content
description: Figure-content VLM critic for the report skill (Epic #328 Phase 4). Vision-language-model pass over every figure in a report version directory, scoring three axes — on-brand palette match against anvil/lib/figures/palette.py, caption-claim grounding, adjacency-claim grounding — and emitting a canonical _review.json (kind=vision) with a critical_figure_misrepresents_claim flag on caption-vs-figure contradiction. Per-figure VLM budget + content-hash → review JSON cache.
---

# report-figure-content — Figure-content VLM critic

**Role**: vision-language-model critic (Epic #328 Phase 4).
**Reads**: `<thread>.{N}/<thread>.pdf` (rendered report) + `<thread>.{N}/figures/` (direct sources).
**Writes**: `<thread>.{N}.figure-content/_review.json` (canonical sibling critic dir).

This command is the report-skill's **figure-content VLM critic** — a vision-language-model pass that scores every figure in a report version directory along three axes (on-brand palette match, caption-claim grounding, adjacency-claim grounding). It is the **fourth phase** of the reframed Epic #328 (Phase 1 / Track A judgment-enrichment shipped in #333 / PR #334; Phase 2 hyperlink-resolver in #335 / PR #338; Phase 3 citation-coverage in #336 / PR #337). Phase 4 is the VLM critic class — shipping alongside Phase 5 #341 (image-accessibility) and Phase 6 #342 (claim-figure-grounding).

**Cross-skill consumer parity.** The implementation lives at `anvil/lib/figure_content.py` and is consumed identically by `memo-figure-content` (the sibling command in the memo skill). Reports tend to carry **more figures** than memos (a full customer-facing report is figure-heavy by design), so the VLM cost cap and content-hash cache are load-bearing for cost control on this consumer specifically. The two-consumer threshold for direct-lib placement was satisfied at ship.
...
**Composability**: `report-figure-content` is **independently re-runnable** and **independently aggregable**. The reviewer (`report-review`) does NOT call this critic directly; instead the operator runs `report-figure-content` and the aggregator auto-discovers the resulting sibling. This decoupling matches the report-skill's existing N-parallel-critics convention.
...
- `version_dir`: the version dir name (e.g., `"customer-report.1"`).
...
python -m anvil.lib.figure_content \
    anvil/skills/report/examples/<example>/<thread>.{N}/
```
...
project/
  customer-report/
    ...
    customer-report.1.review/         <- standard reviewer sibling
      _review.json
    customer-report.1.audit/          <- audit sibling (CUSTOMER-READY)
      _review.json
    customer-report.1.figure-content/ <- THIS critic's sibling
      _review.json
```

When the operator (or a future automated runner) calls `aggregate(reviews)` after loading every sibling, the figure-content findings merge with the reviewer's findings and any `critical_figure_misrepresents_claim` flag short-circuits the verdict to `BLOCK` — load-bearing for the report skill's customer-facing posture.
...
| **No figures at all** | No PDF and no `figures/` dir | Clean pass (no findings, zero VLM calls); top-level `reason` documents the empty state | Expected for figure-light reports; no action needed. |
...
- Re-running `report-figure-content <version_dir>` is byte-equivalent across runs **when the VLM payload is cached or the callback is deterministic**. Live SDK calls introduce model-side nondeterminism (sampling); the cache makes repeat invocations within a session deterministic by construction.
...
## What `report-figure-content` does NOT do

- **Never edit the report body.** The critic is read-only against `<thread>.md`, `<thread>.pdf`, and `figures/`.
...
- **Caption-vs-figure contradiction is the load-bearing surface for customer-facing reports.** The other two axes (on-brand, adjacency-grounding) produce evidence-level findings; the critical flag fires only on the caption case because that is the one a customer will catch and discount the report's credibility for.

- **Figure-heavy reports amortize the content-hash cache.** A 30-page customer report with a header logo on every page costs **1 VLM call** for the logo via the cache, not 30. The cache is internal to `figure_content.py` for now; if Phase 5 (#341) shares the same shape, a follow-on promotion to `anvil/lib/vision_cache.py` lands.
...
- **Commit**: `anvil(report/figure-content): <thread>.{N} [<state>]` (the bracket carries the thread's current derived state per SKILL.md §State machine — specialist critics do not advance the state machine).
