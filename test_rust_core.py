...
try:
    ...
    actions = [[burn_house, 1], [burn_house, 1], [burn_house, 1], [burn_house, 1]]  # All work on burning house
    rewards, done, info = env.step(actions)
    print(f"✅ Step successful: rewards={rewards}, done={done}")

    # Test result
    result = env.get_result()
    print(f"✅ Got result: final_score={result.final_score}, agent_scores={result.agent_scores}")
    ...
    print("cd bucket-brigade-core && PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 pip install -e .")
