%% main.tex --- Bucket Brigade workshop paper (v9 draft)
%%
%% v9 absorbs the k* coordination-threshold vs trainability join (repo
...
and re-verifying the entropy retirement. The replacement
coordination-threshold ($k^{*}$) hypothesis is now also retired in its
binary form: joining a downstream per-cell $k^{*}$ oracle against the
sweep, PPO closes the NE gap on $k^{*}{\geq}2$ columns as well as on
$k^{*}{=}1$ columns (rank-biserial $0.00$, exact Mann--Whitney
$p{=}0.54$, permutation $\Delta$mean $-0.002$)---the second retired
predictor. What survives is weaker and flagged as such: post hoc, the
three $k^{*}{=}k_{\max}$ columns are exactly the untrainable zone
(rank-biserial $-1.00$, exact $p{=}0.0035$), but $k^{*}$ is a pure
function of $\kappa$ on this grid, so the signal is observationally
equivalent to a $\kappa$ effect. On the
...
\texttt{experiments/nash/phase\_diagram/kstar\_vs\_trainability.\{py,json,md\}}).
...
level we report a double negative result---conditional entropy of the
NE profile does not predict gap closure, and no statistic of the NE
profile can, by the $\beta$-invariance argument of
...
$8$ re-measured verdicts); and the replacement coordination-threshold
$k^{*}$ hypothesis is falsified in its binary form by the full-grid
join ($k^{*}{=}1$ and $k^{*}{\geq}2$ columns indistinguishable on the
pooled NE gap: rank-biserial $0.00$, exact $p{=}0.54$). Two intuitive
predictors retired in sequence is itself the finding; the honest
remaining signal---the $k^{*}{=}k_{\max}$ columns are exactly the
untrainable zone---is post hoc and, because $k^{*}$ is a pure function
of $\kappa$ on this grid, observationally equivalent to a $\kappa$
effect. On the
