...
def run_scenario_test(
    scenario_name: str, num_games: int = 20, output_dir: str = "scenario_test"
) -> None:
    """
    ...
    if scenario_name == "trivial_cooperation":
        ...
        firefighters_rank = next(
            (
                r["rank"]
                for r in agent_rankings
                if "firefighter" in agents[r["agent_id"]].name.lower()
            ),
            None,
        )
        free_riders_rank = next(
            (
                r["rank"]
                for r in agent_rankings
        ...
        if (
            firefighters_rank
            and free_riders_rank
            and firefighters_rank < free_riders_rank
        ):
        ...
        print(
            "   Expected: Mixed results - coordinators may outperform pure free riders"
        )
        coordinators = [
            r
            for r in agent_rankings
