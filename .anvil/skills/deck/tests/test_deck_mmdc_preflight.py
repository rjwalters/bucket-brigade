"""Unit tests for the ``mmdc`` (mermaid-cli) preflight (issue #65).

``deck-figures`` must check that ``mmdc`` is on PATH before any
...
``mmdc`` is REQUIRED for any deck with a diagram, not a fallback.
...
Deck-distinct filename per the #58 packaging convention (the deck and slides
...
# anvil/skills/deck/tests/test_deck_mmdc_preflight.py — four levels deep from
# the repo root.
