...
def load_evolved_agent(scenario: str, version: str = "v4") -> Optional[np.ndarray]:
    ...
    if len(genome) != 10:
        raise ValueError(f"Expected 10-parameter genome, got {len(genome)} for {scenario} {version}")

    return genome


def load_all_evolved_agents(scenario: str, versions: List[str] = ["v3", "v4", "v5"]) -> List[np.ndarray]:
    """
...
def is_genome_similar(genome1: np.ndarray, genome2: np.ndarray, threshold: float = 0.1) -> bool:
    """
