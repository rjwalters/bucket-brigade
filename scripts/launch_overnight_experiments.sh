...
tmux send-keys -t overnight-evolution:0 "python experiments/scripts/run_generalist_evolution.py --population $POPULATION --generations $GENERATIONS --games-per-scenario 10 2>&1 | tee logs/evolution/generalist_\$(date +%Y%m%d_%H%M%S).log" C-m
...
echo "  - 9 improved specialist agents"
echo "  - 1 generalist agent"
