%% main.tex --- Bucket Brigade workshop paper (v10 draft)
%%
%% v10 is the pub-revise pass consuming the v9 critic siblings
%% (anvil_pub.bb-workshop.9.review, .9.audit, .9.numeric). Headline
%% changes:
%%   - CF-1 (artifact supersession): every S4 primary-sweep statistic is
%%     recomputed from the artifacts at HEAD (commit a5b8ccdc regenerated
%%     recalibrated_verdict.json and entropy_vs_trainability.{json,md}
%%     with n=20 means blended onto the 8 buy-down cells). Class means
%%     are now 0.142/0.096/0.060/-0.024; the entropy headline is the
%%     HEAD rho = 0.109 (p = 0.56) with the original all-n=4 rho = 0.007
%%     (artifact revision 22b1fda6) carried as provenance; the
%%     "robust to the metric choice" ordering claim is WITHDRAWN (the
%%     homogeneous-metric symmetric/mixed ordering inverts at HEAD:
%%     0.041 vs 0.046); Figure 2 is re-rendered from the HEAD JSON.
%%   - CF-2: the unsampled-cell descriptor is corrected at all three
%%     sites (S3 prose, Table 1 caption, Figure 1 caption): the six
%%     unsampled c=0.5 cells are the kappa in {0.3, 0.7} columns, not a
%%     "high-kappa corner", and the Table 1 caption now matches the
%%     table body (row totals 9/6/9/6/9).
%%   - Five severe overfull hboxes fixed (breakable \path{} artifact
%%     paths via xurl; display-math reward tuple; restructured verdict
%%     ladder; reworked anchor-table provenance column).
%%   - Abstract compressed to ~260 words (statistics moved to their
%%     owning sections); conclusion tightened; related work now engages
%%     the OpenSpiel / exploitability-evaluation tradition and the
%%     "only published" claim is scoped accordingly.
...
\begin{figure}[t]
  ...
  cost $c\in\{0.5,1.0,2.0\}$; within each panel rows are $\kappa$ and
  columns are $\beta$. Each cell is the
...
empirical \textsf{mixed} class---a gap we own and attribute to the named
systematic biases (the $\beta$-independence prediction, while correct,
holds by construction in the sweep's bernoulli extinguish mode and
serves as a solver-noise probe rather than a confirmation). The
$37$-cell PPO trainability sweep, evaluated under a per-cell NE-anchored
metric, recovers the predicted ordering
\textsf{symmetric}~$>$~\textsf{mixed}~$>$~\textsf{asymmetric}~$>$~%
\textsf{collapse}; a separate $4{\times}$-budget sweep on the
no-convergence cells worsens gap-closure, so PPO's failure there is
structural, not budget-limited; and the metric is itself a standalone
methodological finding, because the previously-used single-cell baseline
inverts the per-class ordering wherever the random-policy return varies
across cells. At the per-cell level we report a double negative
result: the entropy predictor is retired---no statistic of the NE
profile can explain within-column variance, by the $\beta$-invariance
argument of \S\ref{sec:trainability}, re-verified by the $20$-seed
noise buy-down---and the replacement coordination-threshold $k^{*}$
hypothesis is falsified in its binary form by the full-grid join. Two
intuitive predictors retired in sequence is itself the finding; the
honest remaining signal ($k^{*}{=}k_{\max}$ columns coincide with the
untrainable zone) is post hoc and observationally equivalent to a
$\kappa$ effect. On the social-trap scenario \texttt{rest\_trap-v1},
the benchmark's hardest committed target, the equilibrium
characterization is now closed, trained policies at the standard budget
are statistically indistinguishable from random play, and a
$16{\times}$-budget ladder yields exactly one marginal, variance-driven
statistical escape with no improvement in the mean---leaving the
${\approx}80$/step scripted headroom as a concrete, machine-verdicted
open challenge. Bucket Brigade is positioned as a methodological
