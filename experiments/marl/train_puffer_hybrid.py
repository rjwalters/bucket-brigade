#!/usr/bin/env python3
"""
Hybrid PufferLib + Custom PPO - True multiprocessing + GPU acceleration.

Uses PufferLib's multiprocessing vectorization for parallel environment steps,
combined with our own GPU-accelerated PPO implementation.

Usage:
    # Full training (10M steps)
    uv run python experiments/marl/train_puffer_hybrid.py \
        --steps 10000000 \
...
def train_ppo_hybrid(
    ...
    """PPO training with PufferLib multiprocessing vectorization."""
    if device is None:
    ...
    while total_env_steps < num_steps:
        ...
        for _ in range(rollout_length):
            ...
            # Step environments (PufferLib handles multiprocessing)
            next_obs, rewards, dones, _ = vec_env.step(actions.cpu().numpy())
        ...
        # === LEARNING PHASE: Update policy on GPU ===
        if len(rollout_obs) > 0:
            # Stack rollout data
            rollout_obs_batch = torch.stack(rollout_obs)
            ...
            with torch.no_grad():
                ...
                T, B = rollout_rewards_batch.shape
...
def main():
    ...
    run_name = args.run_name or f"{args.scenario}_hybrid_{timestamp}"
