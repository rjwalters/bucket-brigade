%% main.tex --- Bucket Brigade workshop paper (v11 draft)
%%
%% v11 is the pub-revise polish pass consuming the v10 critic siblings
%% (anvil_pub.bb-workshop.10.review 38/44 advance:true;
%% .10.audit 0 critical flags; .10.numeric 0 findings; .10.litsearch).
%% Headline changes:
%%   - Figure 1 caption's transposed panel-layout clause fixed: rows are
%%     beta and columns are kappa, matching the rendered figure and its
%%     generating script (the v10 review's single major; the figure
%%     itself was already correct and is untouched).
%%   - S4 wording precision (v10 audit NC-3): the PPO sweep skipped the
%%     splitting Nash row's beta=0.1 CELL (the mixed-verdict cell); the
%%     row remains in the PPO subgrid with two asymmetric_only samples.
%%   - Litsearch integration: the four previously-unverified refs.bib
%%     keys (ppo2017, mappo2022, openspiel2019, psro2017) were
%%     resolver-CONFIRMED by anvil_pub.bb-workshop.10.litsearch (v10
%%     audit NC-1 closes; zero corrections). Seven resolver-verified
%%     entries merged from candidates.bib: S6 gains
%%     lanctot2023population, li2024meta, christianos2022pareto
%%     (the exploitability line's own benchmark move, meta-game
%%     evaluation, and the equilibrium-selection matrix-game tradition
%%     -- each sharpening the parametric-family differentiator);
%%     Appendix A's template subsection gains diekmann1985volunteer,
%%     ledyard1995public, skyrms2003stag, shapley1953stochastic (the
%%     standing v6 carry-forward closes). Claus & Boutilier 1998 stays
%%     uncited: it is an unverified web lead (no resolvable identifier).
%%   - Conclusion cut from ~340 to ~160 words (dim 9): what was
%%     established, what the open challenge is, and the positioning
%%     sentence; every statistic stays in its owning section.
%%   - The submission-time page-budget answer is now stated in-paper (S1
%%     footnote): what the 4-page workshop body keeps vs. what moves to
%%     the supplementary appendix.
%%   - hypertexnames=false silences the duplicate hyperref destination
%%     warnings from the tagged (S)/(A)/(C) display equations (v10 nit).
%% See changelog.md for the full critic-note -> change map.
%%
%% v10 consumed the v9 siblings: every S4 primary-sweep statistic
%% recomputed from the artifacts at HEAD (v9 audit CF-1; class means
%% 0.142/0.096/0.060/-0.024, entropy headline rho=0.109 p=0.56 with
%% rho=0.007 pinned at revision 22b1fda6 as provenance, the
%% metric-robustness claim withdrawn, Figure 2 re-rendered); the
%% unsampled-cell descriptor corrected at all three sites (CF-2); the
%% five severe overfull hboxes fixed to a 0-overfull build; the abstract
%% compressed to ~325 rendered words; S6 gained the OpenSpiel /
%% exploitability-evaluation engagement and the "only published" claim
%% was scoped to the parametric-family property.
%%
%% v9 absorbed the k* coordination-threshold vs trainability join (repo
%% issue #477; PR #475 / issue #430 Task 2; artifact
%% experiments/nash/phase_diagram/kstar_vs_trainability.{py,json,md},
%% built from the full 75-cell thrust#269/#290 per-cell k* artifact --
%% the test v8 reported as "blocked on thrust#269" has now run):
%%   - \S4: the binary threshold prediction (k* = 1 cells trainable,
%%     k* >= 2 cells not) is FALSIFIED for PPO on this grid. Pre-registered
%%     class test on the pooled NE gap, k*=1 (3 columns) vs k*>=2 (8):
%%     rank-biserial 0.00 (U exactly at the null center), exact one-sided
%%     Mann-Whitney p = 0.539, exhaustive permutation Delta-mean = -0.002
%%     (two-sided p = 0.994); power floor 0.006, so perfect separation was
%%     detectable. The v8 "validation pending" paragraph is replaced by
%%     the falsification -- the paper's second retired predictor, and the
%%     negative-results arc is now framed as such.
%%   - The surviving observation -- the three k* = k_max = 4 columns are
%%     exactly the untrainable zone on the homogeneous gap (rank-biserial
%%     -1.00, exact one-sided p = 0.0035 = the combinatorial floor at
%%     3 vs 10; permutation two-sided p = 0.021) -- is carried as
%%     POST-HOC / exploratory at every mention, and flagged as
%%     kappa-confounded everywhere: k* is a pure function of kappa in
%%     this grid (0.1 -> 4, 0.3-0.7 -> 2, 0.9 -> 1), so its predictive
%%     content (Spearman rho = -0.556, p = 0.0004 vs the homogeneous gap
%%     -- the only significant pair in either predictor family, vs the
%%     retired entropy predictor's n.s. rho = 0.109) is observationally
%%     equivalent to "kappa = 0.1 is untrainable"; the NE-gap join is
%%     additionally selection-biased against k* (no_convergence cells
%%     drop out by construction).
%%   - Abstract, \S1 contribution (3c), \S6 Threats, and the Conclusion
%%     updated accordingly; every v8 "unvalidated / pending thrust#269"
%%     k* mention is resolved. No number in the new text quotes a group
%%     median from the artifact's class_comparison display (repo issue
%%     #476: those medians are the upper-middle order statistic for
%%     even-sized groups); means, CIs, rank-biserials, and exact/
%%     permutation p-values are unaffected and used instead.
%% The stale-claim audit table for this revision is committed alongside at
%% stale_claims_audit.md.
%%
%% v8 integrated the three post-v7 results (repo issue #462; PRs #455,
%% #456, #460): the 16x budget-scaling ladder with the first (marginal,
%% variance-driven) escaped_trap verdict on rest_trap-v1, the rest_trap
%% symmetric-DO cycling characterization (12/12 frozen-scenario
%% equilibrium coverage), and the 20-seed noise buy-down (0/8 verdict
%% flips; entropy retirement re-verified at n=20). v7 integrated the
%% July-2026 session results (repo issue #446): the retired entropy
%% predictor + beta-invariance impossibility argument in \S4, the new
%% \S5 social-trap section, the #442 beta-inertness corrections, and the
%% expanded reproducibility subsection. v6 landed the v5-reviewer/auditor
%% punch list. v5 inlined the memo placeholders as Appendices A/B. v9
%% preserves all of those changes.
%%
...
% The tagged display equations (S)/(A)/(C) of S3 share \theequation
% values with Appendix B's numbering, producing duplicate hyperref
% destination warnings (v10 review nit). hypertexnames=false makes
% hyperref use sequential internal destination names instead.
...
\usepackage{array}
% xurl extends the class-loaded url package so that long committed
% artifact paths (\path{...}) can break at any character, eliminating
% the overfull-hbox class of defect flagged in the v9 review.
\usepackage{xurl}
% \path is fragile in moving arguments (captions); predefine the
% caption-resident artifact paths with \urldef.
\urldef\pathRecalVerdict\path{experiments/p3_specialization/phase_diagram_ppo_v2/recalibrated_verdict.json}
\urldef\pathTrapBattery\path{experiments/p3_specialization/scripted_battery/rest_trap.{json,md}}
\urldef\pathTierOneVerdict\path{experiments/p3_specialization/tier1_runs/tier1_verdict.md}
\urldef\pathTrapEscape\path{experiments/p3_specialization/tier1_runs_trap_escape/}
...
no Nash equilibrium is computable for them, so when a new algorithm beats PPO
the reader cannot tell whether it converged to the ``right'' joint policy or
merely to a Pareto-different one. \emph{Bucket Brigade} inverts this trade: a
finite parametric cooperative-competitive stochastic game ($2304$ states and
$8$ actions per agent at the minimal parameterization) whose three
scalars~$(\beta,\kappa,c)$ control equilibrium structure continuously. We
derive closed-form boundary inequalities for three NE regimes via a
single-house mean-field reduction and test them against a $39$-cell empirical
phase diagram: the qualitative phase order
(collapse~$\to$~symmetric~$\to$~mixed~$\to$~asymmetric) is correct, the
quantitative $\kappa$-thresholds are off by $3$--$10{\times}$ (a gap we own
and attribute to three named biases), and a fourth \textsf{mixed} class
emerges that the bound does not predict. A $37$-cell PPO trainability sweep
recovers the predicted class ordering under a per-cell NE-anchored metric
(the previously-used single-cell baseline inverts it---a standalone
methodological lesson). We retire two intuitive trainability predictors in
sequence: conditional entropy of the NE profile does not predict per-cell gap
closure (a $\beta$-invariance argument rules out \emph{every} NE-profile
statistic, and a $20$-seed noise buy-down re-verifies the retirement), and
the replacement coordination-threshold ($k^{*}$) hypothesis is falsified in
its binary form; the surviving $k^{*}{=}k_{\max}$ failure-zone observation is
post hoc and $\kappa$-confounded, and is flagged as such. On the social-trap
scenario \texttt{rest\_trap-v1}, whose equilibrium characterization is
closed, a scripted specialist team demonstrates a machine-verdicted
${\approx}80$/step learnability gap: standard-budget PPO is statistically
indistinguishable from random play, and a $16{\times}$-budget ladder yields
exactly one marginal, variance-driven escape with no mean improvement. We
position Bucket Brigade as a \emph{methodological complement} to the surveyed
benchmarks: to our knowledge the only published \emph{parametric} MARL family
in which ``did the algorithm converge to the right equilibrium?'' admits a
ground-truth answer across a controlled grid.
\end{abstract}
...
grid.\footnote{Page budget: this is the extended version of record; the
target venue's $4$-page workshop body is a strict subset of it. The
$4$-page version keeps \S\ref{sec:intro}, the \S\ref{sec:env}
environment contract, the \S\ref{sec:ne} boundary inequalities with
Figure~\ref{fig:phase}, the \S\ref{sec:trainability} class-ordering
and retired-predictor results with Figure~\ref{fig:ppo}, the
\S\ref{sec:resttrap} anchor ladder (Table~\ref{tab:trap-anchors}),
and the \S\ref{sec:related} positioning, each in compressed form; the
formal specification (Appendix~\ref{app:envspec}), the derivation and
bias accounting (Appendix~\ref{app:nestructure}),
Tables~\ref{tab:phase-counts} and~\ref{tab:bench}, and the detailed
predictor post-mortems of \S\ref{sec:trainability} move to the
supplementary appendix, outside the $4$-page body count.}
...
(3c)~A double negative result on per-cell trainability
predictors~(\S\ref{sec:trainability}): we retire two intuitive
predictors in sequence and identify the honest remaining signal.
First, conditional entropy of the
...
re-verifies the retirement). Second, the replacement
coordination-threshold hypothesis---PPO trains where $k^{*}{=}1$ and
fails where coordinated multi-agent deviation is required---is
falsified in its binary form by the full-grid join against a
downstream per-cell $k^{*}$ oracle: $k^{*}{=}1$ and $k^{*}{\geq}2$
columns are indistinguishable on the pooled NE gap (rank-biserial
$0.00$, exact Mann--Whitney $p{=}0.54$). The remaining signal is
exploratory and we flag it as such at every mention: the
$k^{*}{=}k_{\max}$ columns coincide exactly with the untrainable zone,
but the split is post hoc, and $k^{*}$ is a pure function of $\kappa$
on this grid, so its content is observationally equivalent to a
$\kappa$ effect.
...
templates: the $N$-player Volunteer's
Dilemma~\citep{diekmann1985volunteer} at $H{=}1$; the $N$-player
Public Goods game~\citep{ledyard1995public} when ownership weights
collapse to uniform; Stag Hunt~\citep{skyrms2003stag} in
...
\[
  ...
  p_{\text{own}},p_{\text{other}}) = (10,10,50,0,100,0),
\]
with $\rho{=}0.02$ and
...
c\in\{0.5,1.0,2.0\}$, less the six unsampled $c{=}0.5$ cells at
$\kappa\in\{0.3,0.7\}$---the two mid-$\kappa$ columns of that panel,
across all three $\beta$; Figure~\ref{fig:phase}), recovers the
...
\begin{table}[h]
  ...
  cells at that $\kappa$; the row totals are 9 except at
  $\kappa\in\{0.3, 0.7\}$ (6 cells each: the $c{=}0.5$ panel was not
  sampled at those two $\kappa$ columns). The reduction predicts the
  modal empirical class on $3/5$ $\kappa$ rows. The two off-rows
...
\begin{figure}[t]
  ...
  \texttt{minimal\_specialization} reward tuple. Three panels, one per
  cost $c\in\{0.5,1.0,2.0\}$; within each panel rows are $\beta$ and
  columns are $\kappa$. Each cell is the
  ...
  $\ldots$); the six unsampled cells (the $\kappa\in\{0.3,0.7\}$
  columns of the $c{=}0.5$ panel) are hatched. Four regimes
...
not scheduled---leaving 37 of the 39 Nash-solver cells with PPO data. A
subsequent $20$-seed noise buy-down (seeds $46$--$61$ added to the original
$42$--$45$ at the identical budget; see the buy-down paragraph below) is
blended into the committed verdict artifact, so every per-cell statistic
reported in this section is a mean over $20$ seeds on the $8$ buy-down cells
and over $4$ seeds elsewhere, exactly as committed at
\path{experiments/p3_specialization/phase_diagram_ppo_v2/recalibrated_verdict.json}.
...
\path{experiments/nash/phase_diagram/per_cell_baselines.json} and
computed by \path{bucket_brigade/baselines/per_cell.py}; the recipe is
...
(mean$\pm$std over seeds; $n{=}20$ on the $8$ buy-down cells, $n{=}4$
elsewhere) across the three~$c$ panels. The ordering across
NE classes is
\textsf{symmetric\_only} $(0.142;n{=}11)$ $>$
\textsf{mixed} $(0.096;n{=}9)$ $>$
\textsf{asymmetric\_only} $(0.060;n{=}11)$ $>$
\textsf{no\_convergence} $(-0.024;n{=}6$; the metric falls back to
...
($\approx 0.20$--$0.42$) is far larger than the adjacent class-mean
separations on $\texttt{gap\_closed\_ne}$ ($0.045$ and $0.036$ between
the three NE-bearing classes), so the data is consistent with the
...
the lone splitting Nash row's $\beta{=}0.1$ \emph{cell}---the one
carrying its \textsf{mixed} verdict---so the $(\kappa{=}0.9,
c{=}0.5)$ row enters with two \textsf{asymmetric\_only} samples);
recall from
...
on \texttt{alc-6}):\footnote{The ``$4{\times}$'' here counts training
iterations ($200$ vs.\ $50$); because the rollout length also doubles
($4096$ vs.\ $2048$ steps), the sweep is an $8{\times}$ multiple in
environment steps. The $4{\times}$/$16{\times}$ ladder of
\S\ref{sec:resttrap} multiplies iterations at the \emph{fixed}
$2048$-step rollout, so its labels are true environment-step multiples.
Each label matches its committed artifact's configuration.} mean
$\texttt{gap\_closed\_homogeneous}$ worsens to
$-0.108$ (from $-0.024$ at the original budget). PPO's failure on
...
$\texttt{gap\_closed\_ne}$ gives Spearman $\rho{=}0.109$ ($p{=}0.56$) on
the committed artifact (the original all-$n{=}4$ computation, preserved
at artifact revision \texttt{22b1fda6}, gave $\rho{=}0.007$, $p{=}0.97$;
the $20$-seed buy-down below re-verified the retirement and the
committed headline is the $n{=}20$-blend recomputation---insignificant
either way); all four entropy aggregates (mean, max, min, spread) are
insignificant against both $\texttt{gap\_closed\_ne}$ and
$\texttt{gap\_closed\_homogeneous}$, and the single nominally
significant entry (min vs.\ homogeneous, $p{=}0.038$) does not
...
are committed at
\path{experiments/nash/phase_diagram/entropy_vs_trainability.{py,json,md}}.
...
$\texttt{gap\_closed\_ne}$ varies across $\beta$ by up to $0.34$ within
a column (e.g.\ $-0.32/+0.01/-0.00$ at $\kappa{=}0.1,c{=}1.0$). The
...
equilibrium profile, and the per-cell std of
$\texttt{gap\_closed\_ne}$ reaches $0.99$ (median $0.24$ across the
grid, which is still $n{=}4$ outside the $8$ bought-down cells)---a
noise ceiling that caps the achievable correlation for \emph{any}
predictor tested against these means.
...
\path{experiments/nash/phase_diagram/noise_buydown_precision.md})
...
survives the recomputation with the $n{=}20$ means substituted---the
$\rho{=}0.109$ ($p{=}0.56$) headline quoted there \emph{is} that
recomputation, now the committed artifact's own headline (all four
aggregates remain insignificant). The
$n{=}4$ point estimates were noise-dominated but directionally
right---the largest per-cell shift is $0.372\to 0.043$ at
$(\kappa{=}0.3, c{=}1.0)$---so the class-level conclusions survive the
buy-down (the class means reported above already incorporate the
$n{=}20$ blend) while the original $n{=}4$ error bars, as flagged
above, did not deserve trust.

\paragraph{The coordination-threshold hypothesis, falsified in its
binary form.}
Our working replacement hypothesis was the coordination threshold
$k^{*}$: the minimum coalition size whose simultaneous deviation from
uniform play yields a positive team-return gap, with the conjecture
that PPO---a unilateral best-response method---trains where
$k^{*}{=}1$ and fails wherever coordinated multi-agent deviation is
required ($k^{*}{\geq}2$). That binary prediction has now been tested
in full and is falsified for PPO on this grid. A downstream oracle
supplies per-cell $k^{*}$ for all $75$ cells of its
$(\beta,\kappa,c)$ grid (\texttt{rjwalters/thrust\#269}/\texttt{\#290};
committed verbatim and joined at
\path{experiments/nash/phase_diagram/kstar_vs_trainability.{py,json,md}}).
Because $k^{*}$, like every other quantity in this extinguish mode, is
$\beta$-invariant (byte-identical across $\beta$ within every
$(\kappa,c)$ column), $\beta$-replicas are not independent
observations: all class tests run at the column level, pooling
$\texttt{gap\_closed\_ne}$ across $\beta$ with $n$-seed weights.
Thirteen of the artifact's $15$ effective $(\kappa,c)$ columns have a
PPO sweep and join; two \textsf{no\_convergence} columns among them
have no NE baseline and drop out of the NE-gap outcome, leaving $11$.
The pre-registered class test---%
$k^{*}{=}1$ ($3$ columns) vs.\ $k^{*}{\geq}2$ ($8$ columns) on the
pooled NE gap---finds nothing at all: the Mann--Whitney $U$ sits
exactly at the null center (rank-biserial $0.00$; exact one-sided
$p{=}0.539$), and the exhaustive $165$-assignment permutation gives
$\Delta$mean${}={}-0.002$ (two-sided $p{=}0.994$). The same split on
$\texttt{gap\_closed\_homogeneous}$ (all $13$ columns) is likewise
insignificant (rank-biserial $0.40$, exact one-sided $p{=}0.19$). At
these class sizes only near-perfect separation is detectable (minimum
achievable one-sided $p{=}0.006$), so a modest threshold effect could
hide---but the observed effect is not modest-and-missed, it is
\emph{zero}: PPO closes the NE gap on $k^{*}{=}2$ columns as well as
on $k^{*}{=}1$ columns. The literal ``unilateral-BR methods fail
whenever $k^{*}{>}1$'' account is dead on this grid; this supersedes
the underpowered $2$-vs-$6$ partial-stratification test of the
previous revision ($p{=}0.43$) and makes $k^{*}$'s binary form the
second predictor this paper retires.

\paragraph{What survives the falsification, and what it is confounded
with.}
Post hoc---the split was chosen after seeing the primary null, and we
flag it as exploratory at every mention---the failure zone is
$k^{*}{=}k_{\max}{=}4$: on $\texttt{gap\_closed\_homogeneous}$ (the
only gap defined on \textsf{no\_convergence} cells) the three
$k^{*}{=}4$ columns fall below all ten $k^{*}{\leq}2$ columns
(rank-biserial $-1.00$; exact one-sided $p{=}0.0035$, the
combinatorial floor at $3$ vs.\ $10$; permutation two-sided
$p{=}0.021$). PPO's cliff is not where coordination is first required
($k^{*}{=}2$) but where the \emph{whole team} must deviate at once.
Two structural caveats bound what this can mean. First,
$\kappa$-confounding: in this artifact $k^{*}$ is a pure function of
$\kappa$ ($\kappa{=}0.1{\to}4$, $0.3$--$0.7{\to}2$, $0.9{\to}1$), so
every $k^{*}$ result---including the failure zone and the correlation
below---is observationally equivalent to ``$\kappa{=}0.1$ is
untrainable''; the coalition mechanism is a candidate
\emph{explanation} of the $\kappa$ effect, not independently
identified here. Second, selection bias: \textsf{no\_convergence}
cells have no NE baseline, so they drop out of the NE-gap join by
construction---biased \emph{against} $k^{*}$ exactly where it makes
its strongest prediction. With those flags in place, the comparison to
the retired entropy predictor still favors $k^{*}$: Spearman $k^{*}$
vs.\ $\texttt{gap\_closed\_homogeneous}$ is $\rho{=}-0.556$
($p{=}0.0004$, $n{=}37$ cells; $\rho{=}-0.635$, $p{=}0.020$ at column
level), the only significant pair in either predictor family, against
the entropy predictor's n.s.\ $\rho{=}0.109$/$0.342$. Nor does
$k^{*}$ refine the NE taxonomy: it cross-cuts the verdict classes
(every $k^{*}$ level mixes at least two of them, and
\textsf{asymmetric\_only} cells appear at all three $k^{*}$ levels);
the single clean alignment is that all \textsf{no\_convergence} cells
fall in $k^{*}{=}4$ columns, consistent with the post-hoc failure-zone
reading---$k^{*}$ is a coarser, $\kappa$-driven trainability marker,
not a sharper equilibrium taxonomy. The account must also absorb the earlier awkward datum: a
...
(\path{experiments/nash/phase_diagram/improvability_oracle.md}), so
single-agent improvability alone does not separate trainable from
untrainable cells either. This paper accordingly makes no quantitative
predictor claim; the retirement of the entropy predictor (re-verified
at $n{=}20$), the $\beta$-invariance impossibility argument, and the
falsification of the binary $k^{*}$ threshold are the results we stand
behind---the $k^{*}{=}k_{\max}$ failure zone is offered as an
exploratory, $\kappa$-confounded observation awaiting an out-of-grid
test.
...
metric (per-cell Random~$\to$~SpecialistPolicy~$\times$~4) compresses
the cross-class separation, and the full four-way ordering is
\emph{not} robust to the metric choice: on the committed artifact the
homogeneous class means are
\textsf{symmetric}~$(0.041)$, \textsf{mixed}~$(0.046)$,
\textsf{asymmetric}~$(0.030)$, \textsf{collapse}~$(-0.024)$---the
symmetric/mixed pair inverts relative to the NE-anchored ordering, on a
$0.005$ separation that is well inside the within-class noise. (An
earlier revision claimed the homogeneous metric ``yields the same
qualitative ordering''; that held on the all-$n{=}4$ artifact by a
$0.0005$ margin and inverts under the $n{=}20$ blend, so we withdraw
it.) What survives on both metrics is the coarser, load-bearing
contrast: all three NE-bearing classes sit above
\textsf{no\_convergence}, and \textsf{symmetric\_only} sits above
\textsf{asymmetric\_only}; the \textsf{mixed} class's exact rank is
metric- and noise-sensitive.
...
\begin{figure}[t]
  ...
  ($50$ iterations $\times$ $2048$ rollout steps with
  the in-repo \texttt{JointPPOTrainer}; $20$ seeds on the $8$ buy-down
  cells, $4$ seeds elsewhere, as committed at
  \pathRecalVerdict), arranged in three panels by
  cost~$c\in\{0.5,1.0,2.0\}$. Mean shown above the Tufte-style horizontal
  bar; bar half-width is proportional to per-cell std, scaled so
  the widest std spans 80\% of the cell. Symmetric-only cells (top-left
  of each panel) show the largest gap closure; asymmetric-only cells
  show positive but smaller gap closure; \textsf{mixed}
...
(\path{bucket_brigade/baselines/release/local/nash/rest_trap-v1.json}).
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}l l >{\raggedright\arraybackslash}p{1.55in}@{}}
    ...
    Frozen NE ($3{\times}$FR$+1{\times}$FF) & $\leq 248.67$
      & NE payoff $2984.04$/ep $\div$ $12$ nights \\
...
\path{run_tier1_cell.classify_trap_verdict}) computes the
...
\begin{itemize}
  \item $lo > \texttt{scripted\_best.ci95\_hi}$:
        \textsf{above\_scripted\_best};
  \item $lo > \texttt{random\_ci95\_hi}$: \textsf{escaped\_trap};
  \item $lo > \texttt{ne\_bound}$: \textsf{at\_random};
  \item else \textsf{trapped\_at\_ne}.
\end{itemize}
The \textsf{escaped\_trap} rung deliberately
...
it is.\footnote{Provenance note: the $300/300$ RNG/resample-combination
sweep and the $t$-interval trace to the PR~\#460 judge recomputation as
recorded in this paper's committed revision trail
(\path{paper/anvil_pub.bb-workshop.8/changelog.md},
\path{paper/anvil_pub.bb-workshop.8/stale_claims_audit.md}), not to a
regenerable \path{experiments/} artifact---weaker than this paper's
usual artifact standard, and flagged as such.} \emph{There is no
dose-response on the mean}: the plateau is
...
The honest comparison is that Bucket Brigade dominates on exactly
one column (NE characterisability) and loses on every other (citation
...
\paragraph{The equilibrium-computation and exploitability-evaluation
tradition.} A second, closer line of prior work evaluates
convergence-to-equilibrium directly rather than through benchmark
scores. OpenSpiel~\citep{openspiel2019} ships dozens of small
normal-form and extensive-form games together with exact solvers, so
``did the learner reach equilibrium?'' is answerable there too; the
PSRO/NashConv line~\citep{psro2017} made exploitability the standard
convergence-to-equilibrium metric for learning in games, and our
double-oracle solver and the per-position exploitability bounds of
\S\ref{sec:resttrap} are direct descendants of that machinery. What
that tradition evaluates on, however, is a library of \emph{fixed,
individually-specified} games---mostly zero-sum or classic matrix
games---rather than a single \emph{parametric family} whose
equilibrium \emph{structure} (which regime obtains, not just which
profile) moves continuously under interpretable environment scalars.
The line's own later evaluation work keeps that shape: its
benchmark-style move measures populations on repeated
rock--paper--scissors, a single fixed
game~\citep{lanctot2023population}, and its current methodological
statement for deep MARL estimates equilibria \emph{empirically} from
simulated meta-games~\citep{li2024meta} rather than reading them off
an analytical map. Likewise the oldest form of NE-ground-truth MARL
evaluation---equilibrium \emph{selection} in fixed matrix games whose
equilibria are known by inspection, the climbing/penalty-game
tradition~\citep{christianos2022pareto}---supplies ground truth only
for individually-specified, stateless two-agent matrices.
Bucket Brigade's claim is therefore scoped precisely: it is not the
first environment where equilibrium convergence is measurable (that
credit belongs to the exploitability tradition), but to our knowledge
it is the only published \emph{parametric} cooperative-competitive
MARL family in which a closed-form regime map over the parameter
space---a phase diagram---supplies the ground truth that a trainability
sweep can then be scored against, cell by cell.
...
covering the rest of the grid. The $k^{*}$ join of
\S\ref{sec:trainability} inherits two limits of its own: $k^{*}$ is a
pure function of $\kappa$ on this grid, so no within-grid test can
separate the coalition mechanism from a plain $\kappa$ effect (the
exploratory $k^{*}{=}k_{\max}$ failure-zone reading needs cells where
$k^{*}$ varies within $\kappa$, which the current grid does not
contain), and the class tests carry a power floor (minimum achievable
one-sided $p{=}0.006$ at $3$ vs.\ $8$ columns) that only near-perfect
separation could beat---the primary null is a measured zero effect,
not merely a failure to reach significance. The per-cell-baseline
...
recipe lives in the in-repo \path{docs/PAPER_RESULTS.md}. The per-cell
baselines used by the \S\ref{sec:trainability} metric are produced by
\path{bucket_brigade/baselines/per_cell.py} and serialised at
\path{experiments/nash/phase_diagram/per_cell_baselines.json}; both
...
reproduce byte-for-byte from the committed per-seed summaries,
including the seed-$46$--$61$ buy-down summaries blended into the $8$
extended cells---every \S\ref{sec:trainability} statistic and the
rendered Figure~\ref{fig:ppo} match the artifacts at repository HEAD; a
...
(\texttt{python -m} \path{bucket_brigade.baselines.parity}
\texttt{--all}) re-measures
...
benchmark whose Nash-equilibrium regime map is derivable in closed form
and testable cell by cell, and everything the paper establishes is
scored against that map: the closed-form bound predicts the qualitative
phase order that both the $39$-cell empirical diagram and the $37$-cell
PPO sweep recover (the latter under the per-cell NE-anchored metric of
\S\ref{sec:trainability}; the bound's quantitative
$\kappa$-thresholds miss by $3$--$10{\times}$ and a fourth
\textsf{mixed} class escapes it, both reported as part of the claim);
two intuitive per-cell trainability predictors are retired in sequence;
and on \texttt{rest\_trap-v1} trained policies capture essentially
none of a machine-verdicted ${\approx}80$/step scripted headroom even
at $16{\times}$ budget. The open challenge is equally concrete:
predict the \textsf{mixed} boundary in closed form, find a per-cell
trainability predictor that survives an out-of-grid test, and capture
the committed \texttt{rest\_trap} headroom---each answerable, cell by
cell, against a closed-form regime map that to our knowledge no other
published MARL benchmark family pairs with its training grid. Bucket
Brigade is a methodological complement to the canonical MARL benchmarks
for the specific question of equilibrium-convergence quality, not a
...
Dilemma~\citep{diekmann1985volunteer}. The $N$-player generalisation
at $N>1,H=1$ is the
...
(R_{\text{team}}+P_{\text{team}})-c$, producing the textbook
public-goods underinvestment gradient~\citep{ledyard1995public}.
...
lazy NE where everyone \textsc{Rest}-defects (low payoff, no
coordination required)---the canonical Stag Hunt payoff
structure~\citep{skyrms2003stag}. The intermediate-$\kappa$ regime
interpolates between Stag Hunt and the public-goods regime.
...
Every formal result for finite stochastic
games~\citep{shapley1953stochastic}---minimax, equilibrium existence
in stationary mixed strategies, value iteration convergence---applies
directly. The contribution of the Bucket Brigade
...
Before the full $39$-cell grid of \S\ref{sec:ne} was computed, the
reduction was first checked against a 7-cell preview at
$c{=}0.5,\rho{=}0.02$---the historical record matters because the
preview is where the quantitative threshold disagreements were first
observed and owned, and because the single-cell-baseline inversion of
\S\ref{sec:trainability} was diagnosed on the same preview.
Table~\ref{tab:ne-empirical} reproduces that comparison; the biases it
exposes are dissected in \S\ref{app:nestructure:gaps} below.
...
\paragraph{Small-$q(k)$ linearisation.}
The survival integral of \S\ref{app:nestructure:reduction} approximates
$(1-q(k))^{T_{\min}}\approx 1-T_{\min}\cdot q(k)$, a first-order Taylor
step that is only accurate when $T_{\min}\cdot q(k)\ll 1$. At the
parameter corner where the approximation is worst ($\kappa$ small, so
$q(k)\to\rho{=}0.02$ and $T_{\min}q\approx 0.24$), the linearisation
overstates the expected loss by the neglected
$O\!\left((T_{\min}q)^2\right)$ terms---i.e.\ precisely in the collapse
band where the predicted threshold is already $3$--$10{\times}$ too
low. The bias is smaller than the ring-locality term but acts in the
same region, so it is listed as a distinct source rather than folded
into the survival coefficient.
