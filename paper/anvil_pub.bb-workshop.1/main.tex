%% main.tex --- Bucket Brigade workshop paper (v1 draft)
%%
...
%% The TikZ placeholder in figures/ppo_heatmap_placeholder.tex requires the
%% tikz package, which is loaded below alongside the class defaults.
...
no-pure-NE collapse) via a single-house mean-field reduction, and report the
agreement between predicted and empirically-sampled equilibrium verdicts on a
...
~$\to$~asymmetric) and $\beta$-independence at $c{=}0.5$, but the quantitative
$\kappa$-thresholds disagree with empirics by 3--10$\times$; we own this gap
and attribute it to ring-locality, per-agent ownership rewards, and the
heuristic-strategy approximation. We position Bucket Brigade as a
\emph{methodological complement} to the surveyed benchmarks, not a
replacement: it is the only published parametric MARL game where the question
``did the algorithm converge to the right equilibrium?'' admits a ground-truth
answer.
\end{abstract}
...
(3)~A protocol and falsifiable hypothesis~(\S\ref{sec:trainability}) for
testing PPO trainability against the NE phase diagram; \emph{empirical
results are pending and will land in v2 once the sweep completes}. (4)~An
honest positioning~(\S\ref{sec:related},~\S\ref{sec:discussion}) against the
canonical benchmarks: Bucket Brigade is small and artificial by design, and
is not a substitute for any of them.

We treat the analytical NE characterization as ground truth and the PPO
trainability sweep as a falsification test, in that order. The empirical
hypothesis the paper will defend once~\S\ref{sec:trainability} lands data is:
\emph{PPO success correlates with NE structure}---symmetric-NE cells should
converge reliably; asymmetric-only cells should cycle or settle into
low-payoff mixed strategies; no-pure-NE cells should fail outright.
...
boundary is the paper's claim, not a weakness we hide. Detailed derivations,
the $7\times 4$ predicted-vs-empirical table, and the five sources of
systematic bias appear in~\citep{bbnestructure}.
...
\section{Trainability: empirical protocol and the analytical hypothesis}
\label{sec:trainability}

\textbf{Data status.} The PPO trainability sweep is in progress at the time
of this v1 draft (Bucket Brigade issue~\#360). \emph{This section reports the
experimental protocol we will run and the analytical hypothesis the run will
test; no empirical PPO numbers appear in v1.} v2 of the paper will land the
empirical heatmap once the sweep completes; in the interim,
Figure~\ref{fig:ppo} is the labelled placeholder that the figurer will
replace.

\paragraph{Protocol.}
We train independent runs at each of the 7 phase-diagram cells from
Figure~\ref{fig:phase}, using the \texttt{JointPPOTrainer} default budget
($4$ seeds per cell, default rollout length and update count). The PPO
implementation follows the standard recipe of~\citet{ppo2017}; the
joint-action multi-agent formulation follows the
MAPPO-style centralised-critic setup of~\citet{mappo2022}. The headline
metric is $\texttt{gap\_closed}$ against the analytically-derived
\texttt{Hero} baseline: a run \emph{succeeds} on a cell if it achieves
$\texttt{gap\_closed}\geq 0.20$ within the budget. The reported per-cell
score is the mean success rate over the 4 seeds. Hyper-parameters and the
budget definition are pinned in the in-repo
\texttt{experiments/p3\_specialization} configs.

\paragraph{Analytical hypothesis.}
The closed-form characterisation in~\S\ref{sec:ne} makes a sharp,
falsifiable prediction about which cells PPO will succeed on:

\begin{itemize}
  \item \textsf{symmetric\_only} cells (a single coordinated profile to
  find): PPO should converge reliably (high success rate);
  \item \textsf{asymmetric\_only} cells (the rest-trap; the only NE is
  symmetry-breaking): PPO should struggle---we expect either
  cycle-and-fail or convergence to low-payoff mixed strategies---because
  self-play with symmetric initialisation has no clean attractor toward the
  asymmetric profile;
  \item \textsf{no\_convergence} (collapse) cells: PPO should fail
  outright, because no pure-strategy NE exists for policy iteration to
  converge to.
\end{itemize}

If the empirical PPO success rate per cell matches this ordering---
\textsf{symmetric\_only}~$>$~\textsf{asymmetric\_only}~$>$~%
\textsf{no\_convergence}---the analytical characterisation has predictive
content for algorithm design. If the ordering is violated (e.g.\ PPO succeeds
on collapse cells), that is itself a positive paper result: it would mean
PPO has converged to a non-NE attractor that the analytical reduction does
not see, which would constrain future characterisations.

\begin{figure}[t]
  \centering
  \input{figures/ppo_heatmap_placeholder.tex}
  \caption{\textbf{Placeholder.} PPO per-cell success rate
  ($\texttt{gap\_closed}\geq 0.20$, mean over 4 seeds) across the same
  $(\beta,\kappa)$ grid as Figure~\ref{fig:phase}. \emph{Data pending Bucket
  Brigade issue \#360}; v2 of this paper will replace this stub with the
  rendered heatmap. The analytical prediction is
...
The PPO trainability data is pending~(\S\ref{sec:trainability}). If the
empirical PPO success rate violates the predicted ordering
\textsf{symmetric}~$>$~\textsf{asymmetric}~$>$~\textsf{collapse}, the
analytical hypothesis is falsified; we have committed to reporting this
falsification rather than suppressing it.
...
recipe lives in the in-repo \texttt{docs/PAPER\_RESULTS.md}.
Frozen baseline checkpoints (the analytical \texttt{Hero}, \texttt{Firefighter},
and \texttt{FreeRider} agents used to anchor the phase-diagram solver and
the PPO \texttt{gap\_closed} metric) will be hosted on the HuggingFace Hub;
the publication pathway for the baselines is in progress and not yet
complete at the time of this draft. The source repository is at
...
empirical PPO trainability test is in progress; the analytical hypothesis is
\textsf{symmetric}~$>$~\textsf{asymmetric}~$>$~\textsf{collapse}. Bucket
Brigade is positioned as a methodological complement to the canonical MARL
benchmarks for the specific question of equilibrium-convergence quality, not
